...
env:
  ...
  ROLE_TO_ASSUME: arn:aws:iam::032021926264:role/github-actions-ecr

jobs:
###############################################################################
# Build jobs – identical pattern, just the directory + repo name changes
###############################################################################
  build-consumer:
...
###############################################################################
# Deploy step (unchanged except for indent fix)
###############################################################################
  deploy:
    needs:
      - build-web-main
